...
class RunCoordinator:
    ...
    def _run_with_session(
        ...
        *,
        ...
        config = self._merge_promoted_discovery_sources(session, config)
    ...
    def _merge_promoted_discovery_sources(
        self,
        session: Session,
        config: AppConfig,
    ) -> AppConfig:
        ...
        promoted = discovery_repo.list_promoted_resolutions()
        if not promoted:
            return merged

        source_map = {source.name: source for source in merged.sources.sources}
        base_urls = {
        ...
        param_keys = {
            "greenhouse": "board_tokens",
            "lever": "account_names",
            "ashby": "job_board_names",
        }

        for _, resolution in promoted:
            platform = (resolution.platform or "").lower()
            if platform not in param_keys:
                continue
            ...
            if source is None:
                source = SourceDefinition(
                    ...
                    params={param_keys[platform]: []},
                )
                merged.sources.sources.append(source)
                source_map[platform] = source

            source.enabled = True
            identifiers = source.params.setdefault(param_keys[platform], [])
            if not isinstance(identifiers, list):
                identifiers = []
                source.params[param_keys[platform]] = identifiers

            identifier = resolution.identifier or self._identifier_from_url(resolution.url)
            if identifier and identifier not in identifiers:
                identifiers.append(identifier)
        return merged

    def _identifier_from_url(self, url: str | None) -> str | None:
        if not url:
            return None
        parsed = urlparse(url)
