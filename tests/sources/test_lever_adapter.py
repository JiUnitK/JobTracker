from __future__ import annotations

import json
from pathlib import Path

from jobtracker.config.models import SourceDefinition
from jobtracker.models import SearchQuery
from jobtracker.sources.lever import LeverAdapter


def test_lever_adapter_parses_and_filters_jobs() -> None:
    fixture_path = Path("tests/fixtures/lever_postings.json")
    payload = json.loads(fixture_path.read_text(encoding="utf-8"))
    adapter = LeverAdapter(fetch_json=lambda _: payload)
    source = SourceDefinition(
        name="lever",
        type="ats",
        reliability_tier="tier1",
        params={"account_names": ["exampleco"]},
    )
    query = SearchQuery(
        keywords=["platform engineer", "python"],
        locations=["Austin, TX", "Remote"],
        workplace_types=["remote", "hybrid", "onsite"],
    )

    jobs = adapter.collect(source, query)

    assert len(jobs) == 1
    assert jobs[0].source_job_id == "lev-123"
    assert jobs[0].company_name == "Exampleco"
    assert jobs[0].employment_type == "Full-time"
    assert jobs[0].seniority == "Senior"
    assert "Infrastructure" in jobs[0].raw_tags
