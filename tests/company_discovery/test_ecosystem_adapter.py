...
def test_austin_ecosystem_adapter_fetches_entries_from_url() -> None:
    payload = json.loads(
        (Path("tests/fixtures/austin_ecosystem_entries.json")).read_text(encoding="utf-8")
    )
    adapter = AustinEcosystemDiscoveryAdapter(fetch_json=lambda url: payload)
    source = CompanyDiscoverySourceDefinition(
        name="austin_ecosystem",
        type="ecosystem",
        enabled=True,
        params={"entries_urls": ["https://example.com/austin-ecosystem.json"]},
    )
    query = CompanyDiscoveryQuery(
        keywords=["software engineer", "backend engineer"],
        locations=["Austin, TX"],
...
def test_austin_ecosystem_adapter_fetches_query_driven_entries() -> None:
    payload = {
        "entries": json.loads(
            (Path("tests/fixtures/austin_ecosystem_entries.json")).read_text(encoding="utf-8")
        )
    ...
    adapter = AustinEcosystemDiscoveryAdapter(fetch_json=fetch_json)
    source = CompanyDiscoverySourceDefinition(
        name="austin_ecosystem",
        type="ecosystem",
        enabled=True,
        params={
            "query_url_template": (
                "https://ecosystem.example.test/search?q={query}&keyword={keyword}&location={location}&workplace={workplace_type}"
            ),
    ...
    assert seen_urls == [
        "https://ecosystem.example.test/search?q=backend+engineer+Austin%2C+TX+hybrid&keyword=backend+engineer&location=Austin%2C+TX&workplace=hybrid"
    ]
